...
def read_fits_header(file_path):
    try:
        ...
        with fits.open(file_path) as hdul:
            if 'XTE_SA' in hdul:  # Check for 'XTE_SA' extension
                header = hdul['XTE_SA'].header
                datamode = header.get('DATAMODE', 'N/A')
                ddesc = header.get('DDESC', 'N/A')
                return datamode, ddesc
            else:
                print(f"'XTE_SA' extension not found in {file_path}")
                return 'N/A', 'N/A'
            ...
            for dir_name in dirs:
                ...
                if number_directory_pattern.match(dir_name):
                    ...
                    if os.path.isdir(pca_folder):
                        ...
                        csv_file_path = os.path.join(root, dir_name, 'single_bit_data_summary.csv')
                        xdf_file_path = os.path.join(root, dir_name, 'single_bit.god')

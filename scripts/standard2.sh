...
for source_dir in $source_dirs; do
    ...
    # Check for the standard2 file in full_source_dir
    standard2_file=$(find "${full_source_dir}" -maxdepth 1 -type f -name "FS4a*")
    if [[ -f "$standard2_file" ]]; then
        # Copy the standard2 file to the result directory
        if cp "$standard2_file" "$result_dir/"; then
            echo "Copied 'standard2 file' ($standard2_file) from $source_dir to $result_dir"
            successful_copies=$((successful_copies + 1))
        else
            echo "Failed to copy 'standard2 file' ($standard2_file) from $source_dir to $result_dir"
            failed_copies=$((failed_copies + 1))
        fi
    else
        echo "standard2 file starting with 'FH0e' not found in $source_dir. Skipping."
        failed_copies=$((failed_copies + 1))
    fi
done
